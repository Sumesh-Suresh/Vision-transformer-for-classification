...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.patch_embedding = torch.nn.Linear(in_features=self.patch_dim*self.patch_dim*3,out_features=self.d_model) # TODO (Linear Layer that takes as input a patch and outputs a d_model dimensional vector)
        self.positional_encoding =  PositionalEncoding(embed_dim = self.d_model)# TODO (use the positional encoding from the transformer captioning solution)
        self.fc = torch.nn.Linear(self.d_model, self.num_classes) # TODO (takes as input the embedding corresponding to the [CLS] token and outputs the logits for each class)
        self.cls_token = torch.nn.Embedding(num_embeddings=1,embedding_dim=self.d_model) # TODO (learnable [CLS] token embedding)
    ...
    def patchify(self, images):
        ...
        images = images.unfold(2,self.patch_dim,self.patch_dim).unfold(3,self.patch_dim,self.patch_dim).permute(0,2,3,1,4,5)
        images = images.reshape(N,self.num_patches,-1)
        # print('shape got = ', images.shape)
        # print('shape expected =',N,self.num_patches,3*self.patch_dim**2)
        return images
    ...
    def forward(self, images):
        ...
        inside = torch.zeros((images.shape[0],1),dtype=torch.long).to(self.device)
       
        cls_token = self.cls_token(inside)
        # print('shpae of cls toke : ', cls_token.shape)
        # print('shape of patched embed :', patches_embedded.shape)
        patches_embedded =  torch.cat((cls_token,patches_embedded),dim=1)# TODO (append a CLS token to the beginning of the sequence of patch embeddings)
        # print('shape of concatenated embedding: ',patches_embedded.shape)
